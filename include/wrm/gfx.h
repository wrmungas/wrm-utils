...
typedef struct wrm_gfx_Settings wrm_gfx_Settings;
// window creation arguments
typedef struct wrm_gfx_Window_Info wrm_gfx_Window_Info; 
// data format for meshes and shaders
typedef struct wrm_gfx_Format wrm_gfx_Format;
// single 32-bit integer rgba value
typedef u32 wrm_gfx_RGBAi;
// struct of 4 bytes: r, g, b, a
typedef struct wrm_gfx_RGBA wrm_gfx_RGBA;
// struct of 4 floats: r, g, b, a: used for when OpenGL wants color channel 
// values as floats in the range [0.0f, 1.0f]
typedef struct wrm_gfx_RGBAf wrm_gfx_RGBAf;
// data for creating a shader
typedef struct wrm_gfx_Shader_Info wrm_gfx_Shader_Info;
// data for creating a texture
typedef struct wrm_gfx_Texture_Info wrm_gfx_Texture_Info;
// Arguments for mesh creation
typedef struct wrm_gfx_Mesh_Info wrm_gfx_Mesh_Info;
// Arguments for model creation
typedef struct wrm_gfx_Model_Info wrm_gfx_Model_Info;

/* --- Type definitions ---------------------------------------------------- */

struct wrm_gfx_Settings {
    bool verbose;
    bool test;
    bool errors;
    char *shaders_dir;
...
struct wrm_gfx_Shader_Info {
    wrm_gfx_Format format;
    char *vert;
    char *frag; 
};

struct wrm_gfx_Texture_Info {
...
#define WRM_RGBAi_R_BITS 0xff000000u
#define WRM_RGBAi_G_BITS 0x00ff0000u
#define WRM_RGBAi_B_BITS 0x0000ff00u
#define WRM_RGBAi_A_BITS 0x000000ffu
...
*/
void wrm_gfx_deleteShader(wrm_Ref shader);

// --- TEXTURE ---

/* Creates a texture */
bool wrm_gfx_createTexture(const wrm_gfx_Texture_Info *args, wrm_Ref *dest);
/* 
Update the data of texture, offset from x and y, with the provided pixel
data 
*/
bool wrm_gfx_updateTexture(
    wrm_Ref texture, 
...
void wrm_gfx_debugTexture(wrm_Ref texture);
/* 
...
As long as wrm_gfx_quit() is called this need not be
*/
void wrm_gfx_deleteTexture(wrm_Ref texture);

// --- MESH ---

/* Create a mesh */
bool wrm_gfx_createMesh(const wrm_gfx_Mesh_Info *args, wrm_Ref *dest);
/* 
Clones an existing mesh (useful for changing the mesh of a single entity using
the model without affecting all others
*/
bool wrm_gfx_cloneMesh(wrm_Ref src, wrm_Ref *dest);
/* 
Updates a mesh's data: IMPORTANT: ALL models using this mesh will now use 
the updated version 
*/
bool wrm_gfx_updateMesh(wrm_Ref mesh, const wrm_gfx_Mesh_Info *data);
/* For debugging; prints a mesh's data to `stdout` */
void wrm_gfx_debugMesh(wrm_Ref mesh);
/* 
...
*/
void wrm_gfx_deleteMesh(wrm_Ref mesh);
...
*/
bool wrm_gfx_setModelTransform(
    wrm_Ref model, 
    const vec3 pos, 
    const vec3 rot, 
    const vec3 scale
...
*/
bool wrm_gfx_addModelTransform(
    wrm_Ref model, 
    const vec3 pos, 
    const vec3 rot, 
    const vec3 scale
...
/* Set a model's mesh */
bool wrm_gfx_setModelMesh(wrm_Ref model, wrm_Ref mesh);
/* Set a model's texture*/
bool wrm_gfx_setModelTexture(wrm_Ref model, wrm_Ref texture);
/* Set a model's shader - checks for compatibility with the model's mesh */
bool wrm_gfx_setModelShader(wrm_Ref model, wrm_Ref shader);
/* Toggle model visibility */
void wrm_gfx_setModelShown(wrm_Ref model, bool shown);
/* Toggle visibility of model's children */
void wrm_gfx_setChildrenShown(wrm_Ref model, bool shown);
/* Associates models `child` and `parent` as such */
bool wrm_gfx_addChild(wrm_Ref parent, wrm_Ref child);
/* Orphans model `child` from `parent` */
bool wrm_gfx_removeChild(wrm_Ref parent, wrm_Ref child);
/* creates a default colored test triangle - for testing */
bool wrm_gfx_createTestTriangle(wrm_Ref *dest);
/* creates a default error-textured test cube - for testing */
bool wrm_gfx_createTestCube(wrm_Ref *dest);
/* For debugging; prints a model's data to `stdout` */
void wrm_gfx_debugModel(wrm_Ref model);
/* 
...
inline wrm_gfx_RGBAi wrm_RGBA_fromRGBAf(wrm_gfx_RGBAf rgbaf)
{
